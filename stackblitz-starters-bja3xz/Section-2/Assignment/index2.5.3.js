...
/********************* “TripWithUs” Hotel Chain - 2.5_Assignment - 03 *********************/
...
function sortHotels(hotel1, hotel2, pricing) {
  if (pricing === "high-to-low") {
    return hotel2.price - hotel1.price; // descending order - high to low
  } else {
...
app.get("/hotels/sort/pricing", (req, res) => {
  ...
  let result = sortedHotels.sort((hotel1, hotel2) =>
    sortHotels(hotel1, hotel2, pricing),
  );
...
function sortByReview(hotel1, hotel2, reviews) {
...
app.get("/hotels/sort/reviews", (req, res) => {
  ...
  let sortedHotels = hotels.slice();
  let result = sortedHotels.sort((hotel1, hotel2) =>
    sortByReview(hotel1, hotel2, reviews),
  );
  res.json({ hotels: result });
});
...
function sortByAmenity(hotel, amenity) {
...
app.get("/hotels/filter/amenity", (req, res) => {
  ...
  let sortedHotels = hotels.slice();
  let result = sortedHotels.filter((hotel) => sortByAmenity(hotel, amenity));
  res.json({ hotels: result });
});
...
function sortByCountry(hotel, country){
...
app.get("/hotels/filter/country", (req, res) => {
  ...
  let sortedHotels = hotels.slice();
  let result = sortedHotels.filter((hotel) => sortByCountry(hotel, country));
  res.json({ hotels: result });
});
...
function sortByCategory(hotel, category){
...
app.get("/hotels/filter/category", (req, res) => {
  ...
  let sortedHotels = hotels.slice();
  let result = sortedHotels.filter((hotel) => sortByCategory(hotel, category));  
  res.json({  hotels: result });
});
